...
const METER_IMAGE_SOURCES = {
  '2/4':'app/assets/time-2-4.svg','3/4':'app/assets/time-3-4.svg','4/4':'app/assets/time-4-4.svg',C:'app/assets/Common_time.svg',
  '2/2':'app/assets/time-2-2.svg',cutC:'app/assets/Alla_breve.svg','3/8':'app/assets/time-3-8.svg','6/8':'app/assets/time-6-8.svg','9/8':'app/assets/time-9-8.svg','12/8':'app/assets/time-12-8.svg'
};
...
const PICKUP_ICONS = {4:'app/assets/pickup-eighth.svg',8:'app/assets/pickup-quarter.svg',12:'app/assets/pickup-dotted-quarter.svg',16:'app/assets/pickup-half.svg',24:'app/assets/pickup-dotted-half.svg'};
...
class StartScreen {
  ...
  constructor({ audio, onStart, onUpdate, onOpenProject }) {
    ...
    this.#bind();
    this.#refreshRecents();
    document.addEventListener('pk-recentschanged', () => this.#refreshRecents());
    document.addEventListener('pk-languagechange', () => this.#languageChanged());
  ...
  #bind() {
    const e=this.#els;
    e.recentTrigger.addEventListener('click', ev => { ev.preventDefault(); this.#toggleRecents(); });
    e.recentList.addEventListener('click', ev => this.#recentListClick(ev));
    e.openProjectFileButton.addEventListener('click', ev => { ev.preventDefault(); void this.#audio.unlock(); e.projectFileInput.click(); });
    e.projectFileInput.addEventListener('change', ev => this.#openProjectFile(ev));
  ...
  #localizeKeyOptions(){
    [...this.#els.keySignatureSelect.options].forEach(o => {
  ...
  #keyChoiceName(choice,mode){ return I18N.keySignatureName(choice.tonic,mode); }
  #syncKeyPicker(){
  ...
  #meterAriaLabel(v){
    if(I18N.getLanguage()==='en') return v==='C'?'Common time, 4/4':v==='cutC'?'Cut time, 2/2':`Time signature ${v}`;
    return v==='C'?'C-tahtiosoitus, 4/4':v==='cutC'?'Cut C -tahtiosoitus, 2/2':`Tahtiosoitus ${v}`;
  }
  ...
  #syncPickupOptions(){ const e=this.#els,cap=capacity(e.timeSignatureSelect.value),prev=Number(e.pickupSelect.value)||0,values=(PICKUP_PRESETS[e.timeSignatureSelect.value]||[]).filter(u=>u>0&&u<cap); e.pickupSelect.value=String(values.includes(prev)?prev:0); e.pickupChoices.innerHTML=values.map(u=>`<button class="pickup-choice" type="button" data-units="${u}" aria-label="${I18N.t('pickup')} ${pickupFraction(u)}" aria-pressed="false"><img src="${PICKUP_ICONS[u]}" alt=""></button>`).join(''); this.#syncPickupButtons(); }
  ...
  #toggleRecents(){
    const e=this.#els;
    if(e.recentPanel.hidden){
      e.recentPanel.hidden=false;
      e.recentTrigger.setAttribute('aria-expanded','true');
      this.#refreshRecents();
    } else this.#closeRecents();
  }

  #closeRecents(){
    const e=this.#els;
    e.recentPanel.hidden=true;
    e.recentTrigger.setAttribute('aria-expanded','false');
  }
  ...
  #formatRecentDate(value){
    ...
    try{
      return new Intl.DateTimeFormat(I18N.getLanguage()==='fi'?'fi-FI':'en-GB',{day:'numeric',month:'numeric',year:'numeric',hour:'2-digit',minute:'2-digit'}).format(date);
    }catch(error){ return date.toLocaleString(); }
  }

  async #refreshRecents(){
    const e=this.#els, store=window.PikakirjoitinRecentProjects;
    if(!e.recentList||!store) return;
    let records=[];
    try{ records=await store.list(); }catch(error){ console.error(error); }
    e.recentList.replaceChildren();
    if(!records.length){
      const empty=document.createElement('div'); empty.className='recent-project-empty'; empty.textContent=I18N.t('noRecentProjects'); e.recentList.appendChild(empty); return;
    }
    records.forEach(record=>{
      const row=document.createElement('div'); row.className='recent-project-row'; row.dataset.recentId=record.id;
      const open=document.createElement('button'); open.type='button'; open.className='recent-project-open'; open.dataset.openRecent=record.id;
      const title=document.createElement('span'); title.className='recent-project-title'; title.textContent=record.title||I18N.t('appName');
      const meta=document.createElement('span'); meta.className='recent-project-meta';
      const bits=[]; if(record.composer) bits.push(record.composer); const date=this.#formatRecentDate(record.updatedAt); if(date) bits.push(I18N.t('modified')+' '+date); meta.textContent=bits.join(' · ');
      open.append(title,meta);
      const del=document.createElement('button'); del.type='button'; del.className='recent-project-delete'; del.dataset.deleteRecent=record.id; del.textContent='×'; del.setAttribute('aria-label',I18N.t('removeRecent')); del.title=I18N.t('removeRecent');
      row.append(open,del); e.recentList.appendChild(row);
    });
  }

  async #recentListClick(ev){
    const del=ev.target.closest('[data-delete-recent]');
    if(del){
      ev.preventDefault(); ev.stopPropagation();
      if(!window.confirm(I18N.t('removeRecentConfirm'))) return;
      try{ await window.PikakirjoitinRecentProjects.remove(del.dataset.deleteRecent); await this.#refreshRecents(); document.dispatchEvent(new CustomEvent('pk-recentschanged')); }
      catch(error){ console.error(error); this.#els.status.textContent=I18N.t('recentSaveFailed'); }
      return;
    }
    const open=ev.target.closest('[data-open-recent]');
    if(!open) return;
    ev.preventDefault();
    try{
      const record=await window.PikakirjoitinRecentProjects.get(open.dataset.openRecent);
      if(!record||!record.payload) throw new Error('missing recent project');
      await this.#openProjectPayload(record.payload,{recentId:record.id});
    }catch(error){ console.error(error); this.#els.status.textContent=I18N.t('recentOpenFailed'); }
  }

  async #openProjectPayload(payload,meta){
    const e=this.#els;
    e.status.textContent=I18N.getLanguage()==='en'?'Opening project…':'Avataan projektia…';
    const ok=await this.#audio.unlock();
    if(!ok){ e.status.textContent=I18N.getLanguage()==='en'?'Audio could not be started.':'Äänen avaaminen epäonnistui.'; return; }
    if(window.PikakirjoitinMicCalibration?.calibrate){
      e.status.textContent=I18N.getLanguage()==='en'?'Measuring background noise…':'Mitataan pohjakohinaa…';
      await window.PikakirjoitinMicCalibration.calibrate();
    }
    try{
  ...
  async #openProjectFile(ev){
    const e=this.#els, input=ev.currentTarget, file=input.files&&input.files[0];
    input.value='';
    if(!file) return;
    try{
      const text=await file.text();
      const raw=JSON.parse(text);
      const payload=window.PikakirjoitinRecentProjects.normalizePayload(raw);
      await this.#openProjectPayload(payload,{fromFile:true});
  ...
  async #submit(ev){
    ...
    if(!editing){
      ...
      // Pohjakohinan mittaus on erillinen moduuli, jotta nykyinen
      // nuotinkirjoitus- ja toisto-logiikka pysyy täysin koskemattomana.
      if(window.PikakirjoitinMicCalibration?.calibrate){
        e.status.textContent=I18N.getLanguage()==='en'?'Measuring background noise…':'Mitataan pohjakohinaa…';
        await window.PikakirjoitinMicCalibration.calibrate();
      }
...
window.PikakirjoitinStartScreen = { StartScreen, THEME_DEFINITIONS };
