  ...
  for (const note of notes) {
    ...
    if (note.tupletId) {
      ...
      const segment = copyTupletMetadata(note, {
        ...
        spellingOverride: note.kind === 'rest' ? null : (note.spellingOverride ? { ...note.spellingOverride } : null),
        manualBeamGroup: note.kind === 'rest' ? null : (note.manualBeamGroup || null),
        manualBeamBreakBefore: note.kind === 'rest' ? false : Boolean(note.manualBeamBreakBefore),
    ...
    while (remaining > 1e-7) {
      ...
      for (const piece of pieces) {
        ...
        const segment = {
          ...
          spellingOverride: note.kind === 'rest' ? null : (note.spellingOverride ? { ...note.spellingOverride } : null),
          manualBeamGroup: note.kind === 'rest' ? null : (note.manualBeamGroup || null),
          manualBeamBreakBefore: note.kind === 'rest' ? false : Boolean(note.manualBeamBreakBefore && segmentIndex === 0),
