...
export function buildMusicXml(notes, settings={}){
  ...
  const xmlMeasures=measures.map((measure,index)=>{
    ...
    const tempo=index===0&&settings.tempoText&&!settings.suppressTempoDirection?`<direction placement="above"><direction-type><words>${esc(settings.tempoText)}</words></direction-type></direction>`:'';
