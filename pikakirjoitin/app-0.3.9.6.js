...
const PROJECT_APP_VERSION = '0.3.9.6';
...
function startFlickGesture(ev) {
  ...
  // Äänimoottorin avaus aloitetaan suoraan käyttäjän pointerdown-eleessä ja
  // ennen osoittimen kaappaamista. Näin iPadOS Safari säilyttää ääniluvan.
...
class OscillatorAudioManager {
  ...
  begin(midi) {
    ...
    // unlock() kutsutaan tässä synkronisesti pointerdown-käsittelijästä. Ääni
    // luodaan vasta, kun iPad ilmoittaa AudioContextin todella käyvän.
    void this.unlock().then(ctx => {
      if (!ctx || request.cancelled || this.requests.get(request.id) !== request) return;
