...
const gameAudioModeButtons=[...document.querySelectorAll('[data-game-audio-mode]')];
const f0GameAudioPanel=$('#f0GameAudioPanel'),planeGameAudioPanel=$('#planeGameAudioPanel');
const f0HighNoteSelect=$('#f0HighNoteSelect'),f0LowNoteSelect=$('#f0LowNoteSelect');
const planeOneNoteBtn=$('#planeOneNoteBtn'),planeTwoNoteBtn=$('#planeTwoNoteBtn');
const planeModeButtons=[...document.querySelectorAll('[data-plane-mode]')];
const planeTrainABtn=$('#planeTrainABtn'),planeTrainBBtn=$('#planeTrainBBtn');
const planeTrainingSummary=$('#planeTrainingSummary'),gameAudioStatus=$('#gameAudioStatus');
...
// Hiiri jää aina rinnalle varakontrolliksi. Äänimoottori tuottaa vain
// KORKEA / MATALA / TOGGLE -komentoja samoille pelifunktioille.
let gameAudioMode='mouse';
let planeTrainingMode=1;
const GAME_AUDIO_DEFAULT_HIGH_MIDI=67; // G4
const GAME_AUDIO_DEFAULT_LOW_MIDI=69;  // A4
const gameAudioEngine=window.GameAudioEngine
  ? new window.GameAudioEngine({
      engine:'f0',
      notes:{
        [GAME_AUDIO_DEFAULT_HIGH_MIDI]:'KORKEA',
        [GAME_AUDIO_DEFAULT_LOW_MIDI]:'MATALA'
      },
      planeTrainingMode:1,
      planeAction:'TOGGLE',
      planeActions:{A:'KORKEA',B:'MATALA'}
    })
  : null;
...
function completeGameFinish(){
  ...
  gameRunning=false;
  gameFinishing=false;
  game.classList.remove('mouse-control-active');
  stopEngineAudio();
...
function beginGame(){
  ...
  singleLastAcceptedAt=0;
  clearRecognition();
  // Hiiriversiossa kone on lennossa heti pelin käynnistyttyä.
  plane.classList.remove('idle');
  plane.classList.add('active');
  game.classList.add('mouse-control-active');
  spawnClock=0;
  controlLockUntil=0;
...
// === HIIRI / GAME AUDIO ENGINE 1.1 ===
// Pelin omat äänet käyttävät tätä AudioContextia. GAME AUDIO ENGINE pitää
// mikrofonin ja analyysinsä omassa AudioContextissaan.
async function ensureGameAudioContextOnly(){
  appAudioBackgrounded=document.visibilityState==='hidden';
  if(!ctx){
  ...
  if(ctx.state!=='running'){
    try{await ctx.resume()}catch(err){
      console.warn('Peliäänten AudioContextia ei saatu käyntiin:',err);
      return false;
    }
  }
...
function gameAudioModeName(){
  if(gameAudioMode==='f0')return 'Tarkka sävel';
  if(gameAudioMode==='plane')return 'Joustava ääni';
  return 'Hiiri';
}
function finnishMidiName(midi){
  const names=['C','C♯','D','E♭','E','F','F♯','G','A♭','A','B♭','H'];
  const n=Math.round(Number(midi));
  return `${names[((n%12)+12)%12]}${Math.floor(n/12)-1}`;
}
function populateF0GameNoteSelects(){
  if(!f0HighNoteSelect||!f0LowNoteSelect)return;
  if(f0HighNoteSelect.options.length&&f0LowNoteSelect.options.length)return;
  for(let midi=48;midi<=84;midi++){
    const label=finnishMidiName(midi);
    f0HighNoteSelect.add(new Option(label,String(midi)));
    f0LowNoteSelect.add(new Option(label,String(midi)));
  }
  f0HighNoteSelect.value=String(GAME_AUDIO_DEFAULT_HIGH_MIDI);
  f0LowNoteSelect.value=String(GAME_AUDIO_DEFAULT_LOW_MIDI);
}
function planeTrainingRefs(){
  if(!gameAudioEngine||gameAudioEngine.engineKind!=='plane')return null;
  try{return gameAudioEngine.getTrainingReferences?.()||null}catch{return null}
}
function setGameAudioStatus(text){
  if(gameAudioStatus)gameAudioStatus.textContent=text||'';
}
function updateGameAudioSetupUi(){
  for(const btn of gameAudioModeButtons){
    const active=btn.dataset.gameAudioMode===gameAudioMode;
    btn.classList.toggle('active',active);
    btn.setAttribute('aria-checked',active?'true':'false');
    btn.disabled=!!gameAudioEngine?.training;
  }
  if(f0GameAudioPanel)f0GameAudioPanel.hidden=gameAudioMode!=='f0';
  if(planeGameAudioPanel)planeGameAudioPanel.hidden=gameAudioMode!=='plane';

  for(const btn of planeModeButtons){
    const active=Number(btn.dataset.planeMode)===planeTrainingMode;
    btn.classList.toggle('active',active);
    btn.setAttribute('aria-checked',active?'true':'false');
    btn.disabled=!!gameAudioEngine?.training;
  }

  if(planeTrainBBtn)planeTrainBBtn.hidden=planeTrainingMode!==2;

  const refs=planeTrainingRefs();
  const aReady=!!refs?.A;
  const bReady=!!refs?.B;
  if(planeTrainABtn){
    planeTrainABtn.textContent=planeTrainingMode===1
      ? (aReady?'Sävel ✓ opetettu':'Opeta sävel')
      : (aReady?'Sävel 1 ✓ opetettu':'Opeta sävel 1');
    planeTrainABtn.disabled=!!gameAudioEngine?.training;
  }
  if(planeTrainBBtn){
    planeTrainBBtn.textContent=bReady?'Sävel 2 ✓ opetettu':'Opeta sävel 2';
    planeTrainBBtn.disabled=!!gameAudioEngine?.training;
  }
  if(planeTrainingSummary){
    if(planeTrainingMode===1){
      planeTrainingSummary.textContent=aReady
        ?'Sävel on opetettu. Valmis lentoon.'
        :'Opeta yksi sävel kolmella puhalluksella.';
    }else{
      if(aReady&&bReady)planeTrainingSummary.textContent='Molemmat sävelet on opetettu. Valmis lentoon.';
      else if(aReady)planeTrainingSummary.textContent='Sävel 1 valmis. Opeta vielä sävel 2.';
      else planeTrainingSummary.textContent='Opeta sävel 1 ja sävel 2, kumpikin kolmella puhalluksella.';
    }
  }
  if(controlModeLabel)controlModeLabel.textContent='Ohjaus: '+gameAudioModeName();
}
function setGameAudioMode(mode){
  if(!['mouse','f0','plane'].includes(mode))return;
  if(gameAudioEngine?.training)return;
  if(gameAudioEngine?.running){
    try{gameAudioEngine.stop()}catch{}
  }
  gameAudioMode=mode;
  startError.textContent='';
  if(mode==='mouse')setGameAudioStatus('Hiiri ja kosketus ovat valmiina.');
  if(mode==='f0')setGameAudioStatus('Valitse ylös- ja alas-sävel. Mikrofoni avataan Aloita-painikkeesta.');
  if(mode==='plane')setGameAudioStatus(planeTrainingMode===1?'Opeta ensin yksi sävel.':'Opeta ensin kaksi säveltä.');
  updateGameAudioSetupUi();
}
function setPlaneTrainingModeForGame(mode){
  const next=Number(mode)===2?2:1;
  if(gameAudioEngine?.training)return;
  planeTrainingMode=next;
  if(gameAudioEngine){
    try{gameAudioEngine.setPlaneTrainingMode(next)}catch(err){
      startError.textContent=err?.message||'Opetustilan vaihto epäonnistui.';
    }
  }
  setGameAudioStatus(next===1?'Opeta yksi sävel kolmella puhalluksella.':'Opeta kaksi säveltä, kumpikin kolmella puhalluksella.');
  updateGameAudioSetupUi();
}
async function ensurePlaneGameAudioRunning(){
  if(!gameAudioEngine)throw new Error('GAME AUDIO ENGINE ei latautunut.');
  if(gameAudioEngine.engineKind!=='plane'){
    await gameAudioEngine.setEngine('plane');
  }
  if(gameAudioEngine.planeTrainingMode!==planeTrainingMode){
    gameAudioEngine.setPlaneTrainingMode(planeTrainingMode);
  }
  gameAudioEngine.setPlaneAction('TOGGLE');
  gameAudioEngine.setPlaneActions({A:'KORKEA',B:'MATALA'});
  if(!gameAudioEngine.running)await gameAudioEngine.start();
}
async function startPlaneReferenceTraining(reference){
  if(gameAudioMode!=='plane'||!gameAudioEngine)return;
  startError.textContent='';
  setStartButtonDisabled(true);
  try{
    setGameAudioStatus('Avataan mikrofoni ja kalibroidaan… ole hetki hiljaa.');
    await ensurePlaneGameAudioRunning();
    gameAudioEngine.train(reference);
  }catch(err){
    startError.textContent=err?.message||'Opetus ei käynnistynyt.';
    setGameAudioStatus('Opetus ei käynnistynyt.');
    setStartButtonDisabled(false);
    updateGameAudioSetupUi();
  }
}
async function prepareSelectedGameAudio(){
  if(gameAudioMode==='mouse'){
    if(gameAudioEngine?.running)gameAudioEngine.stop();
    return;
  }
  if(!gameAudioEngine)throw new Error('GAME AUDIO ENGINE ei latautunut.');

  if(gameAudioMode==='f0'){
    const high=Number(f0HighNoteSelect?.value);
    const low=Number(f0LowNoteSelect?.value);
    if(!Number.isFinite(high)||!Number.isFinite(low))throw new Error('Valitse molemmat ohjaussävelet.');
    if(high===low)throw new Error('Ylös- ja alas-sävelen pitää olla eri sävelet.');

    if(gameAudioEngine.engineKind!=='f0')await gameAudioEngine.setEngine('f0');
    gameAudioEngine.setNotes({[high]:'KORKEA',[low]:'MATALA'});
    setGameAudioStatus('Avataan mikrofoni ja kalibroidaan… ole hetki hiljaa.');
    await gameAudioEngine.start();
    return;
  }

  await ensurePlaneGameAudioRunning();
  const refs=gameAudioEngine.getTrainingReferences?.();
  const complete=planeTrainingMode===1?!!refs?.A:(!!refs?.A&&!!refs?.B);
  if(!complete){
    throw new Error(planeTrainingMode===1
      ?'Opeta ensin sävel.'
      :'Opeta ensin sekä sävel 1 että sävel 2.');
  }
}
async function startSelectedGame(){
  ...
  try{
    await prepareSelectedGameAudio();
  }catch(err){
    startError.textContent=err?.message||'Ääniohjauksen käynnistys epäonnistui.';
    setGameAudioStatus('Peli ei vielä käynnistynyt.');
    setStartButtonDisabled(false);
    updateGameAudioSetupUi();
    return;
  }
  ...
  try{
    audioReady=await ensureGameAudioContextOnly();
    if(audioReady)await preloadGameFx();
  }catch(err){
    console.warn('Peliäänten valmistelu epäonnistui:',err);
  }
  ...
  if(audioReady){
    try{await startEngineAudioFromBeginning()}
    catch(err){console.warn('Moottoriääni ei käynnistynyt:',err)}
  }
  setStartButtonDisabled(false);
}

if(gameAudioEngine){
  gameAudioEngine.addEventListener('trigger',event=>{
    if(!gameRunning||gameFinishing)return;
    const action=event.detail?.action;
    if(action==='KORKEA')setRoute('KORKEA');
    else if(action==='MATALA')setRoute('MATALA');
    else if(action==='TOGGLE')toggleSingleSoundRoute();
  });

  gameAudioEngine.addEventListener('state',event=>{
    const state=event.detail?.state;
    if(state==='requesting-microphone')setGameAudioStatus('Pyydetään mikrofonilupaa…');
    if(state==='microphone-ready')setGameAudioStatus('Mikrofoni avattu.');
    if(state==='calibrating')setGameAudioStatus('Kalibroidaan… ole hetki hiljaa.');
    if(state==='running'&&!gameRunning&&!gameAudioEngine.training){
      if(gameAudioMode==='f0')setGameAudioStatus('Mikrofoni valmis. Aloita peli.');
      else if(gameAudioMode==='plane')setGameAudioStatus('Mikrofoni valmis.');
    }
  });
  gameAudioEngine.addEventListener('calibrationprogress',event=>{
    if(gameRunning)return;
    const p=Math.round((event.detail?.progress||0)*100);
    setGameAudioStatus(`Kalibroidaan… ${p} % · ole hetki hiljaa.`);
  });
  gameAudioEngine.addEventListener('trainingstart',event=>{
    const idx=event.detail?.referenceIndex||1;
    const label=planeTrainingMode===1?'Sävel':`Sävel ${idx}`;
    setGameAudioStatus(`${label}: puhalla 1/3 · pidä pieni tauko puhallusten välissä.`);
    setStartButtonDisabled(true);
    updateGameAudioSetupUi();
  });
  gameAudioEngine.addEventListener('trainingprogress',event=>{
    const done=event.detail?.completedBlows||0;
    const idx=event.detail?.referenceIndex||1;
    const label=planeTrainingMode===1?'Sävel':`Sävel ${idx}`;
    if(done<3)setGameAudioStatus(`${label}: puhalla ${done+1}/3 · ${done}/3 tallennettu.`);
  });
  gameAudioEngine.addEventListener('trainingretry',event=>{
    const done=event.detail?.completedBlows||0;
    const idx=event.detail?.referenceIndex||1;
    const label=planeTrainingMode===1?'Sävel':`Sävel ${idx}`;
    setGameAudioStatus(`${label}: puhalla ${done+1}/3 · hieman pidempi ääni.`);
  });
  gameAudioEngine.addEventListener('referencetrained',event=>{
    const detail=event.detail||{};
    setStartButtonDisabled(false);
    updateGameAudioSetupUi();
    if(detail.complete){
      setGameAudioStatus(planeTrainingMode===1?'Sävel opetettu. Valmis lentoon.':'Molemmat sävelet opetettu. Valmis lentoon.');
    }else if(detail.nextReference==='B'){
      setGameAudioStatus('Sävel 1 valmis. Opeta seuraavaksi sävel 2.');
    }
  });
  gameAudioEngine.addEventListener('trained',()=>{
    setStartButtonDisabled(false);
    updateGameAudioSetupUi();
  });
  gameAudioEngine.addEventListener('trainingerror',()=>{
    setStartButtonDisabled(false);
    setGameAudioStatus('Opetus epäonnistui. Yritä uudelleen.');
    updateGameAudioSetupUi();
  });
  gameAudioEngine.addEventListener('error',event=>{
    if(gameRunning)return;
    startError.textContent=event.detail?.error?.message||'Mikrofonivirhe.';
    setStartButtonDisabled(false);
    updateGameAudioSetupUi();
  });
}

// Hiiri/kosketus toimii kaikissa tiloissa rinnalla.
// Yläpuolisko valitsee yläreitin, alapuolisko alareitin.
function handleMouseRoutePointer(event){
  if(!gameRunning||gameFinishing)return;
  if(event.pointerType==='mouse'&&event.button!==0)return;
  if(event.target.closest('button,[role="dialog"],#finishOverlay,#startOverlay,#rotateOverlay,#loadingOverlay'))return;

  const rect=game.getBoundingClientRect();
  if(!rect.height)return;
  const relativeY=(event.clientY-rect.top)/rect.height;
  setRoute(relativeY<0.5?'KORKEA':'MATALA');
}
...
for(const btn of gameAudioModeButtons){
  btn.addEventListener('click',()=>setGameAudioMode(btn.dataset.gameAudioMode));
}
for(const btn of planeModeButtons){
  btn.addEventListener('click',()=>setPlaneTrainingModeForGame(Number(btn.dataset.planeMode)));
}
planeTrainABtn?.addEventListener('click',()=>startPlaneReferenceTraining('A'));
planeTrainBBtn?.addEventListener('click',()=>startPlaneReferenceTraining('B'));
...
instrumentFluteBtn.addEventListener('click',startSelectedGame);
game.addEventListener('pointerdown',handleMouseRoutePointer);
...
finishBtn.addEventListener('click',()=>{
  ...
  startError.textContent='';
  setStartButtonDisabled(false);
  if(gameAudioMode==='mouse')setGameAudioStatus('Hiiri ja kosketus ovat valmiina.');
  else if(gameAudioMode==='f0')setGameAudioStatus('Valitse sävelet ja aloita uusi peli.');
  else setGameAudioStatus('Opetetut sävelet säilyvät. Aloita uusi peli.');
  updateGameAudioSetupUi();
  gameTimer.textContent='1:30';
  gameTimer.classList.remove('warning','urgent');
});
...
populateF0GameNoteSelects();
updateGameAudioSetupUi();
if(!gameAudioEngine){
  for(const btn of gameAudioModeButtons){
    if(btn.dataset.gameAudioMode!=='mouse')btn.disabled=true;
  }
  setGameAudioStatus('GAME AUDIO ENGINE ei latautunut. Hiiriohjaus toimii.');
}
