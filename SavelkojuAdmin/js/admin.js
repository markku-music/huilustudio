...
auth.onAuthStateChanged(async u=>{if(u&&await isAdmin(u)){ $('#login').classList.add('hidden');$('#app').classList.remove('hidden');$('#userLabel').textContent=u.email;const aa=$('#adminAccount');if(aa)aa.textContent=u.email;await Promise.all([loadOverview(),loadPlayers(true),loadLevels(),loadRewards()]);}else{$('#app').classList.add('hidden');$('#login').classList.remove('hidden');if(u)await auth.signOut();}});
...
async function loadOverview(){
  const ps=await db.collection('publicPlayers').get();let rewards=0;ps.forEach(d=>rewards+=Number(d.data().cabinetCount)||0);
  const ss=await db.collectionGroup('sessions').orderBy('createdAt','desc').limit(250).get();
  const sessions=ss.docs.map(d=>({id:d.id,...d.data()}));const today=new Date();today.setHours(0,0,0,0);const todays=sessions.filter(x=>dateOf(x.createdAt)>=today);const perfectToday=todays.filter(x=>x.perfect).length;
  $('#mPlayers').textContent=ps.size;$('#mRewards').textContent=rewards;$('#mSessions').textContent=todays.length;$('#mSessionsAll').textContent='kaikkiaan '+sessions.length;$('#mPerfect').textContent=perfectToday;$('#mPerfectRate').textContent=(todays.length?Math.round(perfectToday/todays.length*100):0)+' %';
  const gs=$('#gameSessions'),gp=$('#gamePlayers');if(gs)gs.textContent=sessions.length;if(gp)gp.textContent=ps.size;
  const hour=new Date().getHours();$('#greeting').textContent=(hour<11?'Huomenta':hour<17?'Hyvää päivää':'Hyvää iltaa')+', Markku';
  $('#recentSessions').innerHTML=sessions.slice(0,10).map(x=>`<div class="activityRow ${sessionClass(x.misses)}"><div class="activityIcon">${x.perfect?'🎯':'🎵'}</div><div><b>${esc(x.displayName||'Pelaaja')}</b><div class="muted">${relativeTime(x.createdAt)} · ${x.score||0}/${x.targets||10} · ${durationText(x.durationMs)}</div></div><span class="badge">${x.perfect?'Virheetön':'Harjoitus'}</span></div>`).join('')||'<p>Ei harjoituksia vielä.</p>';
  const fastest=[...todays].filter(x=>Number(x.durationMs)>0).sort((a,b)=>a.durationMs-b.durationMs)[0];
  const highlights=[];if(fastest)highlights.push(`<div class="highlight"><span>⚡</span><div><b>Päivän nopein</b><small>${esc(fastest.displayName||'Pelaaja')} · ${durationText(fastest.durationMs)}</small></div></div>`);if(perfectToday)highlights.push(`<div class="highlight"><span>🌟</span><div><b>${perfectToday} virheetöntä</b><small>Tänään soitettu tarkasti.</small></div></div>`);highlights.push(`<div class="highlight"><span>🏆</span><div><b>${rewards} palkintoa</b><small>Oppilaiden yhteisessä kokoelmassa.</small></div></div>`);$('#todayHighlights').innerHTML=highlights.join('');
}
...
async function openPlayer(p){
  const fresh=await db.collection('players').doc(p.id).get();if(fresh.exists)p={id:fresh.id,...fresh.data()};
  const ss=await db.collection('players').doc(p.id).collection('sessions').orderBy('createdAt','desc').limit(100).get();const sessions=ss.docs.map(d=>d.data());
  const avatar=p.avatar||fallbackAvatar(p.id),level=levelForPlayer(p),stats=sessionStats(sessions),rewards=p.cabinetIds||[];
  const timed=sessions.filter(x=>Number(x.durationMs)>0);const best=timed.length?Math.min(...timed.map(x=>Number(x.durationMs))):null;
  const spark=timed.slice(0,10).reverse();const max=Math.max(...spark.map(x=>Number(x.durationMs)),1),min=Math.min(...spark.map(x=>Number(x.durationMs)),max);const span=Math.max(1,max-min);const points=spark.map((x,i)=>`${i*(100/Math.max(1,spark.length-1))},${38-((max-Number(x.durationMs))/span)*30}`).join(' ');
  let runningBest=Infinity;const chronological=[...sessions].reverse();const records=new Set();chronological.forEach(x=>{const t=Number(x.durationMs);if(t>0&&t<runningBest){runningBest=t;records.add(x)}});
  const sessionRows=sessions.slice(0,30).map(x=>`<div class="sessionRow ${sessionClass(x.misses)}"><div><b>${relativeTime(x.createdAt)}</b><div class="muted">${fmt(x.createdAt)}</div></div><div class="sessionResult">${x.score||0}/${x.targets||10}</div><div class="sessionMisses">${Number(x.misses)||0} virhettä</div><div class="sessionDuration">${durationText(x.durationMs)} ${records.has(x)?'<span class="recordBadge">🔥 ennätys</span>':''}</div></div>`).join('')||'<div class="muted">Ei harjoituksia vielä.</div>';
  const notesUpdated=p.teacherNotesUpdatedAt?`Viimeksi muokattu ${relativeTime(p.teacherNotesUpdatedAt).toLowerCase()}`:'Ei vielä muistiinpanoja';
  $('#playerDetail').innerHTML=`
    <div class="detailHero"><div class="detailAvatar levelFrame${Number(level.stars)||1}">${esc(avatar)}</div><div class="heroText"><h2>${esc(p.displayName)}</h2><div class="heroStars">${starsFor(p)}</div><div class="levelTitle">${esc(level.name||'')}</div><div class="muted lastPlayed">Viimeksi pelannut ${relativeTime(p.lastPlayedAt).toLowerCase()}</div></div><button id="playAsStudent" class="primary playStudent" type="button">🎮 Pelaa oppilaana</button></div>
    <div class="studentStats"><div class="studentStat"><span>Nykyinen taso</span><b>${'⭐'.repeat(Number(level.stars)||1)}</b></div><div class="studentStat"><span>Paras aika</span><b>${durationText(stats.best)}</b></div><div class="studentStat"><span>Tämän viikon kierrokset</span><b>${stats.thisWeek}</b></div><div class="studentStat"><span>Harjoitteluputki</span><b>🔥 ${stats.streak} pv</b></div></div>
    <div class="progressPanel"><div><span class="eyebrow">Kehitys</span><h3>Viimeiset ${spark.length} ajoitettua kierrosta</h3></div>${spark.length>1?`<svg class="sparkline" viewBox="0 0 100 44" role="img" aria-label="Oppilaan aikojen kehitys"><polyline points="${points}" fill="none" stroke="currentColor" stroke-width="3" vector-effect="non-scaling-stroke"/><line x1="0" y1="40" x2="100" y2="40" stroke="currentColor" opacity=".18"/></svg>`:'<div class="muted">Kuvaaja ilmestyy kahden ajoitetun kierroksen jälkeen.</div>'}</div>
...
$('#exportPlayers').onclick=async()=>{const s=await db.collection('players').orderBy('displayNameLower').get();const rows=[['Nimi','Pelikoodi','Taso','Palkintoja','Viimeksi pelattu']];s.forEach(d=>{const p=d.data();rows.push([p.displayName||'',p.code||'',starsFor(p),(p.cabinetIds||[]).length,fmt(p.lastPlayedAt)])});const csv=rows.map(r=>r.map(v=>'"'+String(v).replace(/"/g,'""')+'"').join(';')).join('\n');const blob=new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='huilustudio-oppilaat.csv';a.click();URL.revokeObjectURL(a.href)};
